...
export async function technicalAnalystNode(state) {
  ...
  // Strip the chart-only time series arrays before sending to the LLM.
  // ohlcv (~124 bars) + sma20Line + sma50Line would consume thousands of tokens
  // but add no analytical value — the LLM already has the computed summary indicators.
  // eslint-disable-next-line no-unused-vars
  const { ohlcv, sma20Line, sma50Line, ...indicatorsForLLM } = data;

  const report = await askLLM(
    TECHNICAL_PROMPT,
    `Company: ${state.companyLongName} (${state.ticker})\nIndicators (JSON):\n${JSON.stringify(indicatorsForLLM, null, 2)}`
  );
  return {
    technicalsData: data,   // full object (including ohlcv) streamed to the UI for the chart
