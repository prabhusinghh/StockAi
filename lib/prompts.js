...
- confidence reflects how strongly the EVIDENCE agrees, not how good the company is.
- 85-100: all three reads (fundamentals, technicals, sentiment) clearly align in one
  direction with no material contradiction. Reserve this for truly one-sided cases.
- 70-84: two of three reads align clearly, and the third is neutral (not opposing).
- 55-69: one read is strong, others are mixed or noisy — meaningful conviction but hedged.
- 40-54: genuinely mixed picture — at least two reads point in opposite directions.
- 25-39: the evidence is thin, contradictory, or most reads are weak/unavailable.
- 0-24: almost no reliable data, or the reads are strongly contradictory.

Verdict rules:
- "Invest": confidence >= 60 AND the net signal across all three reads leans positive.
- "Pass": confidence >= 60 AND the net signal clearly leans negative/risky.
- "Watch": everything else — mixed evidence, insufficient data, or confidence < 60.

NEVER output 90+ unless fundamentals, technicals, AND sentiment all clearly agree with
no material risk flagged by the risk manager. A 50 is only correct when the evidence
is genuinely split down the middle — defaulting to 50 is not acceptable.`;
