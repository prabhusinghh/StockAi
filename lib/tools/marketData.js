...
export async function getTechnicals(ticker) {
  ...
  // ── Build full OHLCV series for the candlestick chart ──────────────
  // timestamps are Unix seconds; lightweight-charts wants "YYYY-MM-DD" strings.
  // NOTE: Yahoo Finance uses "timestamp" (singular), not "timestamps".
  const timestamps = result.timestamp ?? [];
  const quoteBar = result.indicators?.quote?.[0] ?? {};
  const rawOpens   = quoteBar.open   ?? [];
  const rawHighs   = quoteBar.high   ?? [];
  const rawLows    = quoteBar.low    ?? [];
  const rawCloses  = quoteBar.close  ?? [];
  const rawVolumes = quoteBar.volume ?? [];
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    // Skip bars that have any null OHLC value (Yahoo sometimes returns nulls for holidays/halts)
    if (o == null || h == null || l == null || c == null) continue;
    const date = new Date(timestamps[i] * 1000);
    const time = `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, "0")}-${String(date.getUTCDate()).padStart(2, "0")}`;
    ohlcv.push({
      time,
  ...
  // Pre-compute per-bar SMA values for chart overlay lines
  // sma() above works on the full closes array; here we build point-by-point
  const smaLine = (period) => {
    const line = [];
    for (let i = period - 1; i < ohlcv.length; i++) {
      const slice = ohlcv.slice(i - period + 1, i + 1).map((b) => b.close);
      const avg = slice.reduce((a, b) => a + b, 0) / period;
      line.push({ time: ohlcv[i].time, value: Number(avg.toFixed(4)) });
    }
    return line;
  };
  ...
  return {
    ...
    trend: latest > sma(closes, 20) && sma(closes, 20) > (closes.length >= 50 ? sma(closes, 50) : 0)
      ...
      ? "downtrend"
      : "sideways",
    // Full OHLCV series + per-bar SMA lines for the candlestick chart
    ohlcv,
    sma20Line: smaLine(20),
    sma50Line: ohlcv.length >= 50 ? smaLine(50) : [],
  };
}
